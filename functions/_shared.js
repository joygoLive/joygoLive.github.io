...
/** PBKDF2 반복수. **행마다 저장한다** — 나중에 올려도 옛 글이 안 깨지게.
 *
 *  왜 5만인가: Workers 무료 등급의 CPU 한도가 요청당 10ms 다. 실측으로 5만 회가
 *  약 3.7ms, 10만 회가 약 7ms — 10만은 한도에 너무 붙는다. 열쇠가 맞는데도
 *  «에러»로 안 열리는 것이 가장 나쁜 실패라, 여기서는 여유를 산다. 짧은 비밀번호를
 *  지키는 실질은 어차피 반복수보다 **시도 횟수 제한**에서 나온다(open.js). */
export const PASS_ITER = 50000;
